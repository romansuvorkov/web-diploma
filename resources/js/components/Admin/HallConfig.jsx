...
const handleStatusChange = (seat) => {
  ...
  const resetChanges = async () => {
    ...
    const seats = await Api.getShow('seats', halls[activeHallIndex].id);
    setSeats(seats);
    setChangedSeats(seats);
    renderSeatsTable(seats, halls[activeHallIndex].row, halls[activeHallIndex].seats);
  }
  ...
  const handleSetActive = async (hallId) => {
    ...
    const seats = await Api.getShow('seats', hallId);
    setSeats(seats);
    setChangedSeats(seats);
    setRows(halls[index].row);
    setSeatsInRow(halls[index].seats);
    renderSeatsTable(seats, halls[index].row, halls[index].seats);
    setSeatsIsLoaded(true);
  }
...
const seatStatus = ['chair_disabled', 'chair_standart', 'chair_vip']
    ...
    return (
          <div className="conf-step__wrapper">        
            <p className="conf-step__paragraph" onClick={() => console.log(changedSeats)}>Выберите зал для конфигурации:</p>
            <HallBtnContainer name={'config'} active={activeHall} setActive={handleSetActive} />
            <p className="conf-step__paragraph" onClick={() => console.log(seats)}>Укажите количество рядов и максимальное количество кресел в ряду:</p>
            {!isLoaded && <Preloader />}
            {isLoaded && <div className="conf-step__legend">
              {<label className="conf-step__label">Рядов, шт<input type="number" min="1" className="conf-step__input" placeholder="10" name="rows" value={rows} onChange={(e) => handleLabelChange(e)}/></label>}
              <span className="multiplier">x</span>
              {<label className="conf-step__label">Мест, шт<input type="number" min="1" className="conf-step__input" placeholder="10" name="seats"value={seatsInRow} onChange={(e) => handleLabelChange(e)}/></label>}
            </div>}
            ...
            {(!isLoaded || !seatsIsLoaded) ? <Preloader /> : <div className="conf-step__hall" onClick={() => console.log(seatsTable)}>
            ...
            {/* <div className="conf-step__hall" onClick={() => console.log(seatsTable)}>
              <div className="conf-step__hall-wrapper">
                {rows > 0 && seatsInRow > 0 && seatsTable.length > 0 && seatsTable.map((row) => (
                  <div className="conf-step__row" key={row[0].id}>
                    { row.map((o) => (
            ...
            {halls[activeHallIndex].is_active === 0 ? <InterfaceBtnContainer reset={resetChanges} accept={submitChanges}/> : <p className="conf-step__paragraph">Нельзя менять конфигурацию открытого для продажи зала, сначала закройте продажу билетов</p>}
        </div>
    )
